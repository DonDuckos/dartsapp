...
>
> **Direktes Live-Score-Polling vom Gerät (2026-08-14):** Der 15-Min.-Match-Agent-Takt ist für ein aktiv beobachtetes Live-Match zu träge. Solange die Live-Hero-Karte auf dem Home-Screen sichtbar ist, pollt `lib/services/bzzoiro_client.dart` alle 10 Sek. direkt vom Gerät aus die bzzoiro-API (`liveScoreProvider`, `autoDispose` — stoppt automatisch, sobald kein Live-Match mehr angezeigt wird, läuft also nie unbeaufsichtigt im Hintergrund). Der Match-Agent bleibt unverändert für alles andere zuständig: Turnierstart/-ende erkennen, Ergebnisse festhalten, `users`-unabhängige Grunddaten aktuell halten, auch wenn gerade niemand die App offen hat. bzzoiro bietet nur REST (kein WebSocket/SSE) — es bleibt also Polling, nur mit deutlich kürzerer Latenz, weil kein Firestore-Umweg mehr nötig ist. **Sicherheitshinweis:** Der API-Key wird per `--dart-define-from-file=dart_defines.local.json` (gitignored, lokal befüllen) zur Build-Zeit eingebettet und landet damit im kompilierten APK — anders als GitHub-Actions-Secrets verlässt er kontrollierte Infrastruktur und ist aus der APK extrahierbar. Für ein 5-$/Monat-Abo in diesem Freundeskreis-Hobbyprojekt ein bewusst akzeptiertes Risiko.
>
...
- **Direktes Live-Score-Polling vom Gerät** (2026-08-14): `lib/services/bzzoiro_client.dart` + `lib/providers/live_score_provider.dart` — siehe Status-Absatz oben für Details. Build mit echtem Key: `flutter build apk --debug --dart-define-from-file=dart_defines.local.json` (Datei lokal anlegen, Format `{"BZZOIRO_API_KEY": "..."}`, ist gitignored). Ohne die Datei/den Key kompiliert die App weiterhin normal, `liveScoreProvider` liefert dann einfach dauerhaft `null` (kein Polling-Timer, kein Crash) und die Live-Karte zeigt den Firestore-Stand.
...
22. ~~Direktes 10-Sek.-Live-Score-Polling vom Gerät (statt 15-Min.-Match-Agent-Takt) für die sichtbare Live-Karte~~ ✓ erledigt (2026-08-14, `lib/services/bzzoiro_client.dart`)
